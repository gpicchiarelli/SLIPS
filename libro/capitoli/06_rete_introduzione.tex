...
Il pattern \texttt{(persona (nome ?n) (età ?e))} è condiviso.
\end{osservazione}
...
\begin{tikzpicture}[
  ...
  join/.style={diamond, draw=orange!50, fill=orange!10, thick, minimum width=1.5cm, minimum height=1.5cm},
...
\node[join, below=of a1] (j1) {$J_1$};
\node[beta, below=of j1] (b1) {$\beta_1$};

% Beta network - level 2
\node[join, below=of b1] (j2) {$J_2$};
