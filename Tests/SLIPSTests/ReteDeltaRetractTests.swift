...
final class ReteDeltaRetractTests: XCTestCase {
    func testMultiLevelDeltaRetractRemovesOnlyAffectedTokens() {
        ...
        _ = SLIPS.eval(expr: "(defrule r (A v ?x) (B v ?x) (C v ?x) => (printout t \"F\"))")
    ...
    func testRetractPurgesBetaMemoryTokens() {
        ...
        _ = SLIPS.eval(expr: "(defrule r (item id ?i) => (printout t ?i))")
        
        let factValue = SLIPS.eval(expr: "(assert item id 1)")
