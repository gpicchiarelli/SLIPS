...
final class MultifieldFunctionsTests: XCTestCase {
    ...
    func testNthOutOfBounds() throws {
        ...
        let result = SLIPS.eval(expr: "(nth$ 5 (create$ a b))")
    ...
    func testFirstBasic() throws {
        let result = SLIPS.eval(expr: "(first$ (create$ a b c))")
    ...
    func testSubseqFullRange() throws {
        let result = SLIPS.eval(expr: "(subseq$ (create$ a b c d) 1 4)")
    ...
    func testSubseqFirst() throws {
        let result = SLIPS.eval(expr: "(subseq$ (create$ x y z) 1 1)")
    ...
    func testMemberLast() throws {
        let result = SLIPS.eval(expr: "(member$ z (create$ x y z))")
    ...
    func testInsertBeginning() throws {
        let result = SLIPS.eval(expr: "(insert$ (create$ b c) 1 a)")
        XCTAssertEqual(result, .multifield([.symbol("a"), .symbol("b"), .symbol("c")]), 
                       "insert$ all'inizio")
    }
    
    func testInsertMiddle() throws {
        let result = SLIPS.eval(expr: "(insert$ (create$ a c) 2 b)")
        XCTAssertEqual(result, .multifield([.symbol("a"), .symbol("b"), .symbol("c")]), 
                       "insert$ in mezzo")
    }
    
    func testInsertEnd() throws {
        let result = SLIPS.eval(expr: "(insert$ (create$ a b) 3 c)")
        XCTAssertEqual(result, .multifield([.symbol("a"), .symbol("b"), .symbol("c")]), 
                       "insert$ alla fine")
    }
    
    func testInsertMultiple() throws {
        let result = SLIPS.eval(expr: "(insert$ (create$ a d) 2 b c)")
        XCTAssertEqual(result, .multifield([.symbol("a"), .symbol("b"), .symbol("c"), .symbol("d")]), 
                       "insert$ con valori multipli")
    }
    
    func testInsertEmpty() throws {
        let result = SLIPS.eval(expr: "(insert$ (create$) 1 x)")
        XCTAssertEqual(result, .multifield([.symbol("x")]), 
    ...
    func testMultifieldInPattern() throws {
        ...
        _ = SLIPS.eval(expr: "(deftemplate item (multislot tags))")
        _ = SLIPS.eval(expr: "(defrule test-mf (item (tags $?t&:(> (length$ ?t) 0))) => (printout t \"Has tags\" crlf))")
        
        _ = SLIPS.eval(expr: "(assert (item (tags a b c)))")
        _ = SLIPS.eval(expr: "(run)")
