...
public enum Functions {
    public static func registerBuiltins(_ env: inout Environment) {
        ...
        // FASE 2: Funzioni multifield (ref: multifun.c)
        MultifieldFunctions.registerAll(&env)
        
        // FASE 4: Funzioni stringa (ref: strngfun.c)
        StringFunctions.registerAll(&env)
        
        // FASE 4: Funzioni matematiche (ref: emathfun.c)
        MathFunctions.registerAll(&env)
        
        // FASE 4: Funzioni pretty print (ref: pprint.c, crstrtgy.c)
        PrettyPrintFunctions.registerAll(&env)
        
        // FASE 4: Funzioni template (ref: tmpltfun.c)
        TemplateFunctions.registerAll(&env)
        
        // FASE 4: Funzioni I/O (ref: iofun.c)
        IOFunctions.registerAll(&env)
        
        // FASE 4: Funzioni utility (ref: miscfun.c)
        UtilityFunctions.registerAll(&env)
        
        // FASE 4: Funzioni query fatti (ref: factqury.c)
        FactQueryFunctions.registerAll(&env)
...
private func builtin_set_join_default(_ env: inout Environment, _ args: [Value]) throws -> Value {
    guard let s = args.first?.stringValue?.lowercased() else { return .boolean(false) }
    switch s {
    case "true", "on", "1": env.joinActivateDefaultOnStable = true; return .boolean(true)
    case "false", "off", "0": env.joinActivateDefaultOnStable = false; return .boolean(true)
    default: return .boolean(false)
    }
}
