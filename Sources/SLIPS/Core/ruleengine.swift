...
public struct Rule: Codable {
    ...
    /// Modulo a cui appartiene la regola (FASE 3 - Module-aware)
    /// Ref: ruledef.h - struct defrule con module pointer
...
public enum RuleEngine {
    ...
    public static func run(_ env: inout Environment, limit: Int?) -> Int {
        ...
        // Ref: RunCommand in engine.c con focus stack management
        let focusStack = env.getFocusStackNames()
        if !focusStack.isEmpty {
            // Riordina agenda secondo focus stack
